...
struct X64Emitter {
  ...
  void prologue(){
    ...
    buf.emit_u8(0x55);
    buf.emit_u8(0x48); buf.emit_u8(0x89); buf.emit_u8(0xE5);
#if MPLX_WIN
    // sub rsp, 32  (shadow space)
    buf.emit_u8(0x48); buf.emit_u8(0x83); buf.emit_u8(0xEC); buf.emit_u8(0x20);
#endif
    // save callee-saved rbx, r12..r15
    push_rbx();
    buf.emit_u8(0x41); buf.emit_u8(0x54); // push r12
    buf.emit_u8(0x41); buf.emit_u8(0x55); // push r13
    buf.emit_u8(0x41); buf.emit_u8(0x56); // push r14
    buf.emit_u8(0x41); buf.emit_u8(0x57); // push r15
  }
  void epilogue(){
    // restore r15..r12
    buf.emit_u8(0x41); buf.emit_u8(0x5F); // pop r15
    buf.emit_u8(0x41); buf.emit_u8(0x5E); // pop r14
    buf.emit_u8(0x41); buf.emit_u8(0x5D); // pop r13
    buf.emit_u8(0x41); buf.emit_u8(0x5C); // pop r12
    // restore rbx
    pop_rbx();
#if MPLX_WIN
    // add rsp, 32 (shadow space)
    buf.emit_u8(0x48); buf.emit_u8(0x83); buf.emit_u8(0xC4); buf.emit_u8(0x20);
#endif
    // mov rsp, rbp; pop rbp
    buf.emit_u8(0x48); buf.emit_u8(0x89); buf.emit_u8(0xEC);
    buf.emit_u8(0x5D);
  }
  // mov rax, imm64
  ...
  void push_rbx(){ buf.emit_u8(0x53); }
  // pop rbx
  void pop_rbx(){ buf.emit_u8(0x5B); }
  // ret
