...
static fs::path manifest() {
  return "mplx.json";
}
static fs::path lockfile() {
  return "mplx.lock";
}
...
int main(int argc, char **argv) {
  ...
  if (cmd == "init") {
    if (fs::exists(manifest())) {
      std::cerr << "mplx.json already exists\n";
      return 1;
    }
    write_manifest();
    return 0;
  ...
  if (!fs::exists(manifest())) {
    std::cerr << "no mplx.json, run 'mplx-pkg init'\n";
    return 1;
  }

  if (cmd == "add") {
    if (argc < 3) {
      std::cerr << "mplx-pkg add <name>@<version>\n";
      return 2;
    }
